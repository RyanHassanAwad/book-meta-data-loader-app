...
# ── Load credentials from secrets if missing locally ──
if not Path("cred.json").exists():
    ...
    if _raw:
        ...
        if _pk:
            _b64_lines = [
                l for l in _pk.replace("\r", "").split("\n")
        ...
        _os.environ["GOOGLE_CREDENTIALS"] = _json.dumps(_data)
